...
app.directive('auditSessions', ['Session', '$routeParams', '$location', 'Client', 'Location', '$q', '$timeout', 'gettextCatalog', 'pagination_labels', function(Session, $routeParams, $location, Client, Location, $q, $timeout, gettextCatalog, pagination_labels) {
  ...
  var link = function( scope, element, attrs ) {
    ...
    scope.visitClient = function(session) {
      Location.get({id: session.location_id}, function(location) {
        $location.path('/locations/' + location.slug + '/clients/' + session.client_mac);
      }, function(err){
        console.log(err);
      });
    };
