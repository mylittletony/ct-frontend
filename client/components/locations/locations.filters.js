...
app.filter('mbps', ['gettextCatalog', function(gettextCatalog) {
  return function(bytes, precision) {
    if (isNaN(parseFloat(bytes)) || !isFinite(bytes)) {
      return '-';
    }
    if (bytes === 0) {
      return gettextCatalog.getString('0 Mbps');
    }
    if (typeof precision === 'undefined') {
      precision = 1;
    }
    var units = [gettextCatalog.getString('Kbps'), gettextCatalog.getString('Mbps'), gettextCatalog.getString('Gbps'), gettextCatalog.getString('Tbps')],
